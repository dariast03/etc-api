...
@ApiTags('ADMIN Plan Estudio')
export class PlanEstudioController {
  constructor(private readonly planEstudioService: PlanEstudioService) {}

  @Post()
  async create(
    @Body() createPlanEstudioDto: CreatePlanEstudioDto,
    @Query('crudQuery') crudQuery: string,
  ) {
    const created = await this.planEstudioService.create(createPlanEstudioDto, {
      crudQuery,
    });
    return created;
  }
  ...
  async update(
    ...
    const updated = await this.planEstudioService.update(
      id,

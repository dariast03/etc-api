...
@ApiTags('ADMIN Oferta Materia')
export class OfertaMateriaController {
  constructor(private readonly ofertaMateriaService: OfertaMateriaService) {}

  @Post()
  async create(
    @Body() createOfertaMateriaDto: CreateOfertaMateriaDto,
    @Query('crudQuery') crudQuery: string,
  ) {
    const created = await this.ofertaMateriaService.create(
      createOfertaMateriaDto,
      { crudQuery },
    );
    return created;
  }
  ...
  async update(
    ...
    const updated = await this.ofertaMateriaService.update(
      id,
      updateOfertaMateriaDto,
      { crudQuery },
    );
    return updated;
  }

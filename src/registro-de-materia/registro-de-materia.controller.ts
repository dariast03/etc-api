...
export class RegistroDeMateriaController {
  constructor(private readonly registroDeMateriaService: RegistroDeMateriaService) {}

  @Post()
  async create(@Body() createRegistroDeMateriaDto: CreateRegistroDeMateriaDto, @Query('crudQuery') crudQuery: string) {
    const created = await this.registroDeMateriaService.create(createRegistroDeMateriaDto, { crudQuery });
    return created;
  }
  ...
  async findOne(@Param('id') id: string, @Query('crudQuery') crudQuery: string) {
    const match = await this.registroDeMateriaService.findOne(id, { crudQuery });
    return match;
  }
  ...
  async update(
    ...
    const updated = await this.registroDeMateriaService.update(id, updateRegistroDeMateriaDto, { crudQuery });
    return updated;
  }
